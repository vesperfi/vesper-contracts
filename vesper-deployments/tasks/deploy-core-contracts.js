...
function validatePoolConfig(poolConfig, targetChain) {
  // Fail fast
  if (!poolConfig) {
    throw new Error(`Missing "${hre.poolName}" configuration in vesper-commons/config/${targetChain}/poolConfig file`)
  }
...
async function shouldReuseImplementation(network, versionInfo, config) {
  ...
  const contractName = config.contract === 'VETH' ? 'VPool' : config.contract
  const command = `cat ../${config.path}/${contractName}.sol | grep 'VERSION' | awk '{ print $6 }'`
...
task('deploy-core-contracts', 'Deploy Vesper core contracts')
  ...
  .setAction(async function ({
    ...
    hre.poolConfig = require(`vesper-commons/config/${targetChain}/poolConfig`)[pool]
