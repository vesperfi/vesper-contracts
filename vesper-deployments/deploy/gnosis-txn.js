...
const getProvider = () => new ethers.providers.JsonRpcProvider(process.env.NODE_URL)

const getGnosisContract = safe => new ethers.Contract(safe, gnosisAbi, getProvider())
...
const gnosisProposeTx = async (safe, tx, targetChain) =>
  axios.post(`${getBaseUrl(targetChain)}/api/v1/safes/${safe}/multisig-transactions/`, tx)
...
async function proposeMultiTxn(safe, targetChain, deployer, multisigNonce = 0, transactions, multiSendAddress) {
  const txn = encodeMulti(transactions, multiSendAddress)
  const baseTxn = {
