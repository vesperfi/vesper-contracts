...
async function shouldMigrateStrategies() {
  ...
  async function migrateAndAssert(oldStrategy, newStrategy, receiptToken) {
    ...
    expect(totalSupplyAfter).to.be.eq(totalSupplyBefore, 'pool total supply after migration is not correct')
    ...
    expect(totalValueAfter).to.be.closeTo(
      ...
      'pool total value after migration is not correct',
    )
    expect(totalDebtAfter).to.be.closeTo(
      totalDebtBefore,
      totalDebtBefore.div('1000'), // 0.1% as delta
      'pool total debt after migration is not correct',
    )
    expect(totalDebtRatioAfter).to.be.eq(totalDebtRatioBefore, 'pool total debt ratio after migration is not correct')

    expect(receiptTokenAfter).to.be.eq(0, 'Receipt token balance of old strategy after migration is not correct')

    expect(receiptTokenAfter2).to.be.closeTo(
      receiptTokenBefore,
      receiptTokenBefore.div('1000'), // 0.1% as delta
      'Receipt token balance of new strategy after migration is not correct',
    )
  }
