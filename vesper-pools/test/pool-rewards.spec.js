...
describe('Rewards for VDAI Pool', function () {
  let vdai, dai, vsp, poolRewards, poolRewardsImpl, accountant
  ...
  beforeEach(async function () {
    ...
    vdai = await deployContract(VDAI.contractName, VDAI.poolParams)
    accountant = await deployContract('PoolAccountant')
    await accountant.init(vdai.address)
    await vdai.initialize(...VDAI.poolParams, accountant.address)

    dai = await ethers.getContractAt('ERC20', await vdai.token())
    vsp = await deployContract('TestVSP', [])
  ...
  describe('Multi reward token', function () {
    ...
    beforeEach(async function () {
      ...
      const uniBalance = ethers.utils.parseEther('2000')
      await adjustBalance(uni.address, poolRewards.address, uniBalance)
    ...
    it('Should distribute for multiple epoch', async function () {
      ...
      let uniBalance = ethers.utils.parseEther('2000')
      await adjustBalance(uni.address, poolRewards.address, uniBalance)
      ...
      await vsp.connect(governor).mint(poolRewards.address, TOTAL_REWARD)
      // Swap and transfer UNI to poolRewards
      uniBalance = ethers.utils.parseEther('2000')
      await adjustBalance(uni.address, poolRewards.address, uniBalance)
      await poolRewards
        .connect(governor)
        [`${notifyMultiSignature}`](
    ...
    it('Should notify reward for valid token only', async function () {
      // Swap and transfer to poolReward
      const usdcBalance = ethers.utils.parseUnits('2000', 6)
      await adjustBalance(uni.address, poolRewards.address, usdcBalance)
