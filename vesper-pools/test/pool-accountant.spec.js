...
describe('Pool accountant proxy', function () {
  ...
  const strategyConfig = { debtRatio: 9000, externalDepositFee: 500 }
  ...
  beforeEach(async function () {
    ...
    strategy = { address: ethers.utils.getContractAddress({ from: user1.address, nonce: 50 }) }
  })

  describe('Add strategy tests', function () {
    it('Should add strategy', async function () {
      const tx = accountant.connect(governor).addStrategy(strategy.address, ...Object.values(strategyConfig))
      await expect(tx)
        .to.emit(accountant, 'StrategyAdded')
        .withArgs(strategy.address, ...Object.values(strategyConfig))
      await expect(tx)
        .to.emit(accountant, 'UpdatedPoolExternalDepositFee')
        .withArgs(0, strategyConfig.externalDepositFee)
      ...
      expect(await accountant.externalDepositFee()).to.eq(
        strategyConfig.externalDepositFee,
        'Incorrect pool external deposit fee',
      )
    })

    it('Should revert if strategy is already active', async function () {
      await accountant.connect(governor).addStrategy(strategy.address, ...Object.values(strategyConfig))
      const tx = accountant.connect(governor).addStrategy(strategy.address, ...Object.values(strategyConfig))
    ...
    it('Should revert if strategy address is zero', async function () {
      const tx = accountant.connect(governor).addStrategy(Address.ZERO, ...Object.values(strategyConfig))
  ...
  describe('Update external deposit fee tests', function () {
    ...
    it('Should revert if strategy is not active', async function () {
      ...
      const fakeStrategy = ethers.utils.getContractAddress({ from: user1.address, nonce: 51 })
      const tx = accountant.connect(governor).updateExternalDepositFee(fakeStrategy, 600)
    ...
    it('Should update external deposit fee', async function () {
      ...
      await expect(tx)
        ...
        .withArgs(strategy.address, strategyConfig.externalDepositFee, 800)
  ...
  describe('Pool external deposit fee tests', function () {
    ...
    beforeEach(async function () {
      ...
      strategy2 = { address: ethers.utils.getContractAddress({ from: user1.address, nonce: 60 }) }
    })
